...
class OgnAscentNodeDatabase(og.Database):
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:deltaSimulationTime', 'double', 0, 'Simulation Delta Time', "Description.", {}, True, 0., False, ''),
        ('inputs:deltaSystemTime', 'double', 0, 'System Delta Time', "Description.", {}, True, 0., False, ''),
        ('inputs:dronePrim', 'target', 0, 'Drone Prim', "Description.", {}, True, None, False, ''),
        ('outputs:execOut', 'execution', 0, None, 'Signal to the graph that execution can continue downstream.', {}, True, None, False, ''),
        ('inputs:domain_id', 'uchar', 0, 'Domain ID', "Description.", {}, True, 0, False, ''),
        ('inputs:nodeNamespace', 'string', 0, 'Node Namespace', "Description.", {}, True, None, False, ''),
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"execIn", "deltaSimulationTime", "deltaSystemTime", "dronePrim", "domain_id", "nodeNamespace", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        ...
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            ...
            print('init 3')
            self._batchedReadAttributes = [self._attributes.execIn, self._attributes.deltaSimulationTime, self._attributes.deltaSystemTime, self._attributes.dronePrim, self._attributes.domain_id, self._attributes.nodeNamespace]
            print('init 4')
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnAscentNodeDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnAscentNodeDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnAscentNodeDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
    ...
    class abi:
        ...
        @staticmethod
        def get_node_type():
            get_node_type_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'get_node_type', None)
        ...
        def compute(context, node):
                ...
                if db.inputs.dronePrim:
                    ...
                    if dronekit_connection == None:
                        if not initialized:
                                ...
                                sitl_tool.launch()
                                def f():
                                    drone_sim_dict[node_id]['dronekit_connection'] = dronekit.connect(sitl_tool.get_dronekit_address(),
                                                                                                      wait_ready=True, timeout=999999,
                                                                                                      rate=120)
            ...
            try:
                compute_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'compute', None)
        ...
        def initialize(context, node):
            ...
            initialize_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'initialize', None)
        ...
        @staticmethod
        def release(node):
            release_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'release', None)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnAscentNodeDatabase._release_per_node_instance_data(node, graph_instance_id)

        @staticmethod
        def update_node_version(context, node, old_version, new_version):
            update_node_version_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'update_node_version', None)
        ...
        def initialize_type(node_type):
            initialize_type_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'initialize_type', None)
            ...
            if needs_initializing:
                ...
                node_type.set_metadata(ogn.MetadataKeys.DESCRIPTION, "Runs the Ascent Spirit SITL, mavproxy and mavros.")
                node_type.set_metadata(ogn.MetadataKeys.LANGUAGE, "Python")
                icon_path = carb.tokens.get_tokens_interface().resolve("${airlab.airstack}")
        ...
        def on_connection_type_resolve(node):
            on_connection_type_resolve_function = getattr(OgnAscentNodeDatabase.NODE_TYPE_CLASS, 'on_connection_type_resolve', None)
