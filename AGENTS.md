...
| [place-people-in-tornado-scenes](.agents/skills/place-people-in-tornado-scenes) | Survivors, diggers, trapped and thrown figures in a post-tornado scene — the epidemiology behind every share, the T+30-60 min epoch, why the wildfire people model does NOT transfer, and the bench that is the only way to verify a partially-buried figure |
| [bake-tree-and-debris](.agents/skills/bake-tree-and-debris) | Baking the TREE half of a damage archetype library so nothing floats — which debris gets physics and which is only laid flat (`SIM_ALL_DEBRIS`), why a branchy limb needs a convex decomposition and a stick does not (`SETTLE_DECOMP_M`), the drop-datum bug that lifted every seated log, and the KEEP_PHYSICS / bake-in-place loop (a settle does NOT reproduce) |
| [build-earthquake-scenes](.agents/skills/build-earthquake-scenes) | Earthquake damage to URBAN buildings — state after three review rounds: the fit-out, the per-construction-type ladders (masonry on a brick lattice, concrete as prisms and rafts, curtain-wall bands with the cage kept, lean-and-sink with a ground response, building pairs), solid walls before fracture, the merged archetype bake, the multi-city assembly by magnitude, the headless runner, every knob, the bug catalogue — and the diversity gate (damage the monoliths next) |
...
| [author-human-poses](.agents/skills/author-human-poses) | Authoring POSES for the RP rigs — why paper-derived joint angles rendered grotesque twice, the empirical pose-row method (one render settles axis/sign), the shipped per-class pose table, pose↔inset coupling, composed-geometry acceptance (solve-file z is not evidence), and the people bench as the only oracle |
...
| [run-osmo-baseline-mission](.agents/skills/run-osmo-baseline-mission) | Submitting and babysitting an OSMO baseline mission without losing the pod or the run — the `make_rerun_mission.py` `environment_order` bug that crashes a mission before it runs a single cell (and leaves the pod idle holding a GPU for hours with the workflow still reporting RUNNING), why PID 60 is not always `mission_launcher.sh` on every pod and must be identified by name, and the multi-tenant GPU isolation gap where device index 0 is not always the pod's own card and none of `NVIDIA_VISIBLE_DEVICES` / Compose `device_ids` / `CUDA_VISIBLE_DEVICES` cleanly fixes it |
...
| [fix-floating-debris](.agents/skills/fix-floating-debris) | Why debris floats and how it was actually fixed — the `UsdGeom.BBoxCache` blind spot that made airborne wood audit as clean, the points-based seating that replaced it, the five debris populations and who owns each, and the review discipline that would have found it in one round |
| [freeze-portable-scenes](.agents/skills/freeze-portable-scenes) | Freezing or baking a scene so it WORKS on another machine — the three defects that shipped in all 18 cells and cost a benchmark sweep (no sky, build-machine asset paths that a Nucleus-anchored layer resolves against the SERVER, cross-scope material bindings that render grey), why a finished cell can never be re-serialised, the `make_portable` pre-flatten pass and the `verify()` gate that blocks all three |
| [freeze-dataset-state](.agents/skills/freeze-dataset-state) | What is ACTUALLY in `final_disaster_dataset/` today — baked positions with Nucleus-referenced looks, the deactivated review camera, why the collect step is off, and the punch list for public release |
| [freeze-disaster-dataset](.agents/skills/freeze-disaster-dataset) | The PLAN and tracking record for `final_disaster_dataset/` — the 4 x 2 x 3 x 5 matrix of frozen 1 km scenes, the directory contract, the per-disaster intensity ladders, the `GT_hints.json` class vocabulary, the self-contained-USD export design (and why `stage.Flatten()` is the wrong tool), and the per-cell status of what exists |
| [export-hurricane-suburban](.agents/skills/export-hurricane-suburban) | Taking the Hurricane/Suburban cells to `final_disaster_dataset/` — the three 1 km level presets, the eight-level house ladder and the `_HOUSE_CUTS`/`intensity_field` mismatch that forced level 3 to 80 m/s, the 24 cladding archetypes that MUST be baked or levels 1-2 silently render undamaged, why `freeze_dataset_launch_script.py` cannot drive this pipeline, the post-review scene model (green stripped foliage, bent trees, fences, street furniture, casualties under debris), and the bug catalogue |
...
| [generate-urban-city](.agents/skills/generate-urban-city) | Building a road-networked downtown with the REAL generator (`build_city` + `city_layout` + `districts` + `city_detail`) instead of hand-rolling a packer — the preset/asset-set stack, restricting `usds.buildings.intact` to chosen assets, OSM-calibrated block and road-hierarchy knobs, height sampling, and the traps that silently flatten the grid or render nothing |
| [urban-layout](.agents/skills/urban-layout) | Placing BUILDINGS correctly in the generated city — measuring which elevations are modelled, the corner/end/mid placement classes, per-asset `yaw-offset` so a mixed-pack pool can be oriented at all, terrace vs pack and the block sizing that decides which fires, the building libraries, the greenery blacklist, and building props / fire escapes |
| [build-hurricane-scenes](.agents/skills/build-hurricane-scenes) | HURRICANE scenes — the near-uniform wind field whose variance comes from BUILDINGS not position, the roof-first ladder that needs four new states BELOW where the tornado ladder starts, why slab-swept-clean is surge and never wind, and the six-layer static water design (no fluid sim) with the corrected water material |
| [build-urban-fire-scenes](.agents/skills/build-urban-fire-scenes) | STRUCTURE fire on urban kit buildings — the F0-F5 compartment-fire ladder (climbing, not sweeping; soot above openings; the shell stands, it does not burn away), per-construction-type recipes, the `soot_plume` fire-event soot model (flames and scorch from ONE event list: EC1 flame, Heskestad plume, thermophoretic deposition), `kit_substitute`'s per-pack routing for whole-asset city buildings, and the bug catalogue: the `Scope.Define` trap that snapped kit buildings to the origin, the world-baked `_cyl` rebar that flew 205 m under a 6 m/s cap, `fractionalCutoutOpacity`'s two required forms, contiguous flame runs, and the Flow emitter budget that renders a city with no smoke while reporting success |
| [build-urban-fire-city](.agents/skills/build-urban-fire-city) | The end-to-end PROCEDURE for a full burning-downtown cell — seven numbered stages, each with the exact command, its env knobs and defaults, where the outputs land (host vs container), the gate to pass and the trap that costs a run: what to CLEAR (the GAC kit cache, `FB_OUT` accumulation, and every gac/dtc bake made before 2026-09-02, which carries mirrored-atlas soot and roof-piercing columns); the layout dump (`FC_INTACT_ONLY=1 FC_DUMP=`) and `gen_burnability_table.py --prove`; the manifest (`fire_city_dry_run.py` + `fire_city_union.py --auto/--seeds/--max-records`) and the crop / `FC_CROP_WINDOW` / `x_orig` frame rule; `fire_city_bake.sh` over the four bake kinds — gac, dtc, kit, and the new `aec` brownstone rows that burn by NAME through `aec_burn` instead of being sliced — with the per-level settle tiers and what a good bake log says; the assembly's frame guard, block audit, emitter budget and the Flow-OOM grep that separates a burning city from one with no smoke and every count right; then people, the layout audit and the freeze — plus (2026-09-02) why `PYTHONHASHSEED=0`/`SG_INSTANCE_PLACEMENTS=1` must match on Stage 1/4/7 alike or the layout itself desyncs, the freeze launcher's missing `--no-window` and its `fire_people` crop-vs-full-dump bug, and the vtk-disappears-inside-Kit first-boot trap |
| [urban-fire-city-layout-gaps](.agents/skills/urban-fire-city-layout-gaps) | OPEN as of 2026-09-02, `Fire/Urban` deleted from Nucleus+local pending this: the shipped city ships far fewer occupied blocks than its own ring/typology mix implies (~70% of the full 1.5 km plate's blocks, ~54% of a 1 km crop window's blocks, carry zero buildings) and `GT_hints.json`'s own `region_m` reports the uncropped 1.5 km box, not the shipped 1 km window — the exact repro numbers/commands, what `test_city_layout_audit.py` already proves correct (the `record_xy` frame fix, NOT this), and the precise open questions (true block denominator from `build_city` itself, where `GT_hints.json`'s `meta.region_m` gets stamped, whether fire-substitute buildings crossing a block edge is a real bug or realistic debris-apron spillover) |
| [slice-buildings-into-kits](.agents/skills/slice-buildings-into-kits) | Cutting a MERGED whole-asset building (GreatAmericanCity, downtowncity, AEC brownstone) into kit pieces the fire/quake ladders can damage — the window-measured storey grid, the per-asset cut-offset search, VTK clipping that carries UVs and materials, the 3x3 ring partition, the piece BUDGET that lands 188-275 pieces per building instead of thousands, the guaranteed roof piece, the acceptance tests, the five silent bugs that each render as a flat brown box, and the disaster-agnostic per-building bake + damage pipeline (region-cut slice, pre-slice texture skin, settle-alone, static export, sidecar) any future disaster (earthquake, tornado, hurricane) reuses on top of it |
| [build-scenes-on-osmo](.agents/skills/build-scenes-on-osmo) | Building and rendering scene_gen scenes on an OSMO dev pod when the local Isaac Sim is busy — the scene-authoring resource shape, getting UNCOMMITTED files onto the pod without a git push, the ~4.4 GB of assets the pod's clone is missing, running detached instead of driving the tmux pane, and the capture path that does not segfault on a headless container |
| [reassemble-buildings-in-frame](.agents/skills/reassemble-buildings-in-frame) | Putting a baked, sliced, swapped or kit-built building BACK into the city in the right frame — the three frame bugs that all read as "buildings on top of the road" + "empty blocks" (a cropped manifest composed in the full-city frame, a bake swapped under the intact cell's 0.01 scale and centroid shift, a world-space centring delta under a yawed holder), why GAC's corner pivots make it the canary, the guards, and the offline audit + unit tests that gate a cell without Isaac |
