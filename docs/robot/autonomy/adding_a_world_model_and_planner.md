...
- **Local planner** — a perpetual node plus a `NavigateTask` server that consumes the [`global_plan` (spec §4)](interface_conventions.md#4-global_plan-global-waypoint-path), a world model input (a lidar voxel map, disparity, or point clouds), and the trajectory controller's `look_ahead`/`tracking_point`, and emits short collision-free trajectories on the [trajectory-controller surface (spec §5)](interface_conventions.md#5-trajectory-group-the-trajectory-controllers-contract-onboard-only) (`trajectory_controller/trajectory_segment_to_add` or `trajectory_override`, `airstack_msgs/msg/TrajectoryXYZVYaw`). References: the default [MIGHTY](../../modules/mighty.md) module's `mighty_bridge` (a thin adapter serving `NavigateTask` in front of an existing planner) and [DROAN](../../modules/droan.md) (a native AirStack planner; [overview](local/planning/index.md)). Spec §5 is **onboard-only** — a local planner can never run offboard.
...
**Local world models** feed the local planner a fast short-range obstacle representation ([overview](local/world_model/index.md)). The default is MIGHTY's acl-mapping voxel map (`global_mapper_ros`, in the [asm_mighty](https://github.com/castacks/asm_mighty) module): a sliding-window voxel map of occupied and unknown space built from the filtered lidar cloud, consumed by the planner over an exposed grid-topic seam. The other worked example is DROAN's disparity pipeline (in the [asm_droan](https://github.com/castacks/asm_droan) module): `disparity_expansion` (C-space expansion of stereo disparity by the robot radius) → `disparity_graph` (rolling window of expanded-disparity keyframes with camera poses) → `disparity_graph_cost_map` (a `cost_map_interface` plugin the CPU DROAN planner loads via its `cost_map` parameter to score candidate trajectories). The GPU planner `droan_gl` does the expansion and graph internally on the GPU and consumes raw disparity directly.

**Be honest about the local contract: there isn't a spec-level one.** Unlike `global_plan` or the trajectory group, the local world-model ↔ planner interface is **not** an interchange in the [Interface Conventions Specification](interface_conventions.md) — a local planner and its world model are a **matched pair**, wired together in the stack entry. Compare the reference stacks: `full_default` includes one `mighty_module.launch.xml` (mapper + planner + bridge as a unit, the mapper→planner grid seam exposed as launch args), `full_droan` includes `droan_gl` alone (disparity in from `perception/stereo_image_proc/disparity`, world model internal), while `full_droan_cpu` includes `droan_local_planner` **plus** `disparity_expansion`, the planner consuming the expansion clouds by relative name in the shared `droan` namespace. Adding a new local world model therefore usually means adapting a planner to consume it (e.g. implementing DROAN's `cost_map_interface` plugin API, or feeding MIGHTY's `mighty_occupancy_grid_topic` seam) or bringing a paired planner with it.
...
Planner variants are named stacks, not launch arguments — the [single-locus rule](../../development/stacks.md#the-single-locus-rule-and-its-lint) puts every wiring deviation in the stack entry file. The worked swap examples are [full_droan](../../../stacks/full_droan/README.md) — byte-identical to `full_default` except the local-planner block, where the MIGHTY module's `mighty_module.launch.xml` include becomes the `asm_droan` module's `droan_gl.launch.xml` — and [full_droan_cpu](../../../stacks/full_droan_cpu/README.md), where that block is two lines:
...
Do the same for yours: `airstack stack new full_default full_my_planner`, then in `stacks/full_my_planner/launch/stack.launch.xml` replace the MIGHTY include with your planner's include (plus any world-model include it needs). Only deviations from canonical names appear as include args — see how `full_macvo` passes exactly one (`droan_gl_disparity_topic`). Every reference planner ships as an **external module**: the stack's `modules.repos` pins it (`full_default` pins the [mighty module](../../modules/mighty.md), `full_droan` the [droan module](../../modules/droan.md)) and `airstack up` syncs the pin when it is missing — do the same for a module-shipped planner of yours.

**Verify:** `airstack up --stack full_my_planner --sim isaac --robots 1 && airstack ready` succeeds and `ros2 node list` shows your planner in place of the `mighty/*` nodes.
...
- **Local:** there is no spec surface to hit — produce the representation your **paired planner** consumes. Two worked pairings: MIGHTY consumes occupied/unknown voxel-centre clouds on its `mighty_occupancy_grid_topic` / `mighty_unknown_grid_topic` launch args, so a world model that publishes those (or a constraint node interposed on the mapper→planner seam) needs no planner change; DROAN's `disparity_expansion` publishes expansion clouds the CPU planner reads by relative name, while the graph + cost map reach it as a `cost_map_interface` plugin selected by its `cost_map` parameter. A different planner means adapting it to your representation (or writing one — Path A).
