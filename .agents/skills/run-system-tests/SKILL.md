...
- **`tests/robot/`** and **`tests/sim/`** — Hermetic **unit** tests (`@pytest.mark.unit`). These are **thin proxy files** that call ``register_unit_tests()`` to re-export tests from each ROS 2 package's own co-located `test/` directory (the ROS 2 / colcon convention) so `pytest tests/` discovers them. The proxy pattern keeps test source next to the code it tests while making it discoverable from the root suite.
