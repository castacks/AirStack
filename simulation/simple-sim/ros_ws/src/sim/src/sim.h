...
public:
  ...
  Sim(float image_fov, int image_width, int image_height, float baseline, std::string model_filename, float model_scale);
  ~Sim();
...
Sim::Sim(float image_fov, int image_width, int image_height, float baseline, std::string model_filename, float model_scale)
  : image_fov(image_fov)
  ...
  world_mesh = NULL;
  if(render_fbx)
    world_mesh = new FBXMesh(model_filename, model_scale);
    //world_mesh = new FBXMesh("/data/fire_academy/fire_academy_no_box.fbx", 0.01);
  drone_mesh = new ColladaMesh("model.dae");
}
...
float Sim::step(std::vector<unsigned char>& left_image_bytes, std::vector<unsigned char>& right_image_bytes){
  ...
  sim_time += deltaTime; // TODO have a fixed dt and multiply an int by that
