...
Optional:
    ...
    DRONE_ENABLE_ZED_SEMANTIC_SEGMENTATION=true
    DRONE_ENABLE_ZED_BBOX_2D=true
    DRONE_ENABLE_OUSTER=true
    MESH_PLANE_ALBEDO_PATH=/path/to/albedo.png
    MESH_PLANE_SIZE_SEED=1234
    MESH_PLANE_POSITION_SEED=5678
...
# ---------------------------------------------------------------------------
HARDCODED_SCENE_USD_PATH = "omniverse://airlab-nucleus.andrew.cmu.edu/Users/ruiwang3/Rui_Office_Exp6.usd"
...
HARDCODED_PERSON_TARGET = [-3.4, -10.5, 0.0]
...
HARDCODED_MESH_PLANE_TARGET_PRIM_NAME = "Cube_01"
HARDCODED_CUBE_01_CENTER = [-4.33508, -9.03413, 1.36348]
HARDCODED_CUBE_01_SCALE = [1.51589, 1.65878, 2.77017]
HARDCODED_MESH_PLANE_NEGATIVE_X_FACE_OFFSET = 0.01
HARDCODED_MESH_PLANE_SIZE_RANGE = [0.5, 1.0]
HARDCODED_MESH_PLANE_SIZE_SEED = 1234
HARDCODED_MESH_PLANE_POSITION_SEED = 5678
HARDCODED_MESH_PLANE_ORIENTATION_DEG = [0.0, 90.0, 0.0]
# Use a path mounted into the Isaac Sim container, or an omniverse:// URL.
HARDCODED_MESH_PLANE_ALBEDO_PATH = "omniverse://airlab-nucleus.andrew.cmu.edu/Users/ruiwang3/sim_patch_office_tracking_scene2.png"
...
HARDCODED_DRONE_INIT_POS = [-7.5, -6.3, 0.07]
# yaw = -90 deg
...
HARDCODED_DRONE_ENABLE_ZED_SEMANTIC_SEGMENTATION = True
...
def sample_patch_on_cube_01_negative_x_face():
    size_seed = int(os.environ.get(
        "MESH_PLANE_SIZE_SEED",
        str(HARDCODED_MESH_PLANE_SIZE_SEED),
    ))
    position_seed = int(os.environ.get(
        "MESH_PLANE_POSITION_SEED",
        str(HARDCODED_MESH_PLANE_POSITION_SEED),
    ))
    size_rng = np.random.default_rng(size_seed)
    position_rng = np.random.default_rng(position_seed)

    min_size, max_size = [float(value) for value in HARDCODED_MESH_PLANE_SIZE_RANGE]
    patch_size = float(size_rng.uniform(min_size, max_size))
    half_patch = patch_size * 0.5

    cube_center = [float(value) for value in HARDCODED_CUBE_01_CENTER]
    cube_half_extent = [float(value) * 0.5 for value in HARDCODED_CUBE_01_SCALE]
    bounds_min = [
        cube_center[index] - cube_half_extent[index]
    ...
    if bounds_max[1] - bounds_min[1] < patch_size or bounds_max[2] - bounds_min[2] < patch_size:
        raise RuntimeError(
            f"Sampled patch size {patch_size:.3f} does not fit within "
            f"{HARDCODED_MESH_PLANE_TARGET_PRIM_NAME}'s YZ face bounds: "
            f"min={bounds_min}, max={bounds_max}"
        )

    x = bounds_min[0] - float(HARDCODED_MESH_PLANE_NEGATIVE_X_FACE_OFFSET)
    y = float(position_rng.uniform(
        bounds_min[1] + half_patch,
        bounds_max[1] - half_patch,
    ))
    z = float(position_rng.uniform(
        bounds_min[2] + half_patch,
        bounds_max[2] - half_patch,
    ))

    return [x, y, z], [patch_size, patch_size], bounds_min, bounds_max, size_seed, position_seed
    ...
    def __init__(self):
        ...
        if HARDCODED_MESH_PLANE_ENABLED:
            ...
            (
                ...
                mesh_plane_size_seed,
                mesh_plane_position_seed,
            ) = sample_patch_on_cube_01_negative_x_face()
            carb.log_warn(
                f"Creating mesh plane '{HARDCODED_MESH_PLANE_PATH}' with OmniPBR "
                f"albedo map: {albedo_path or '<none>'}; "
                f"target={HARDCODED_MESH_PLANE_TARGET_PRIM_NAME}, "
                f"bounds_min={target_bounds_min}, "
                f"bounds_max={target_bounds_max}, center={mesh_plane_center}, "
                f"orientation_deg={HARDCODED_MESH_PLANE_ORIENTATION_DEG}, "
                f"size={mesh_plane_size}, size_seed={mesh_plane_size_seed}, "
                f"position_seed={mesh_plane_position_seed}"
            )
        ...
        if drone_enabled:
            ...
            if enable_zed:
                ...
                enable_semantic_segmentation = os.environ.get(
                    "DRONE_ENABLE_ZED_SEMANTIC_SEGMENTATION",
                    str(HARDCODED_DRONE_ENABLE_ZED_SEMANTIC_SEGMENTATION),
                ).lower() == "true"
                ...
                add_zed_stereo_camera_subgraph(
                    ...
                    enable_semantic_segmentation=enable_semantic_segmentation,
