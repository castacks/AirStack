#!/usr/bin/env python
"""
Single-drone PX4 launcher in the Full Warehouse environment.

Same as example_one_px4_pegasus_launch_script.py but uses the Isaac Nucleus
Full Warehouse scene instead of the default grid.  Good for testing the
SUPER planner adapter against a realistic indoor obstacle layout.

Usage (inside Isaac Sim container):
    python warehouse_px4_pegasus_launch_script.py
"""
...
# --------------------- CONFIGURATION ---------------------
ENV_URL = SIMULATION_ENVIRONMENTS["Full Warehouse"]

# Warehouse assets are in centimeters — scale to meters
STAGE_SCALE = 0.01

# Spawn the drone slightly above the warehouse floor at the entrance aisle
DRONE_INIT_POS = [0.0, 0.0, 0.5]

SAVE_SCENE_TO = None  # set to a dir path to export a self-contained USD
...
class PegasusApp:
    ...
    def __init__(self):
        ...
        if stage_prim.IsValid():
            ...
            carb.log_warn("/World/stage not found — skipping scale and collision setup.")

        add_dome_light(stage)

        if SAVE_SCENE_TO:
            import tempfile
            tmp_usd = os.path.join(tempfile.gettempdir(), "warehouse_prepared.usd")
