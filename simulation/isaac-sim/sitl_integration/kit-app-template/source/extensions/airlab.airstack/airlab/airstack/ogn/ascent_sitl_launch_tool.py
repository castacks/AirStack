...
class AscentSitlLaunchTool:
    ...
    def __init__(self, sitl_dir, vehicle_id, domain_id, node_namespace):
        ...
        self.node_namespace = str(node_namespace)
        self.session_name = AscentSitlLaunchTool.get_tmux_session_prefix() + str(self.domain_id)
        ...
        self.sitl_script = '/usr/bin/bash ' + self.sitl_dir + '/ascent_sitl_tmux.bash ' + \
            ...
            ' ' + str(self.session_name) + ' ' + str(self.domain_id) + ' ' + self.node_namespace
    ...
    @staticmethod
    def kill_all_ascent_sessions():
        process = subprocess.Popen('tmux ls', shell=True,
