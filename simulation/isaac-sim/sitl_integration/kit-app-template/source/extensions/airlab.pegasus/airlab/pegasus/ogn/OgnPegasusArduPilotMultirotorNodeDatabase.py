"""Support for simplified access to data on nodes of type airlab.pegasus.PegasusArduPilotMultirotorNode
...
"""Python module for OGN node type airlab.pegasus.PegasusArduPilotMultirotor. DO NOT EDIT MANUALLY."""
...
class OgnPegasusArduPilotMultirotorNodeDatabase(og.Database):    # Imprint the generator and target ABI versions in the file for JIT generation
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {
            "execIn", "dronePrim", "vehicleID", "robotModel", "connectionType", "connectionIP", "connectionBaseport",
        ...
        @property
        def robotModel(self): return self._batchedReadValues[ArduPilotInputIndex.ROBOT_MODEL]
        @robotModel.setter
        def robotModel(self, value): self._batchedReadValues[ArduPilotInputIndex.ROBOT_MODEL] = value
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnPegasusArduPilotMultirotorNodeDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnPegasusArduPilotMultirotorNodeDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnPegasusArduPilotMultirotorNodeDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
    ...
    class abi:
        ...
        @staticmethod
        def get_node_type():
            get_node_type_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "get_node_type", None)
            if callable(get_node_type_function):
                return get_node_type_function()
            return "airlab.pegasus.PegasusArduPilotMultirotorNode"
        ...
        def compute(context, node):
            ...
            try:
                per_node_data = OgnPegasusArduPilotMultirotorNodeDatabase.PER_NODE_DATA[node.node_id()]
                db = per_node_data.get("_db")
                if db is None:
                    db = OgnPegasusArduPilotMultirotorNodeDatabase(node)
            ...
            try:
                compute_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "compute", None)
                ...
                with og.in_compute():
                    return OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS.compute(db)
        ...
        def initialize(context, node):
            OgnPegasusArduPilotMultirotorNodeDatabase._initialize_per_node_data(node)
            initialize_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "initialize", None)
        ...
        @staticmethod
        def release(node):
            release_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "release", None)
            if callable(release_function):
                release_function(node)
            OgnPegasusArduPilotMultirotorNodeDatabase._release_per_node_data(node)

        @staticmethod
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "init_instance", None)
            if callable(init_instance_function):
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "release_instance", None)
            if callable(release_instance_function):
                release_instance_function(node, graph_instance_id)
            OgnPegasusArduPilotMultirotorNodeDatabase._release_per_node_instance_data(node, graph_instance_id)

        @staticmethod
        def update_node_version(context, node, old_version, new_version):
            update_node_version_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "update_node_version", None)
        ...
        def initialize_type(node_type):
            initialize_type_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "initialize_type", None)
            ...
            if needs_initializing:
                ...
                icon_path = carb.tokens.get_tokens_interface().resolve("${airlab.pegasus}")
                icon_path = icon_path + "/" + "ogn/icons/airlab.pegasus.PegasusArduPilotMultirotorNode.svg"
                ...
                OgnPegasusArduPilotMultirotorNodeDatabase.INTERFACE.add_to_node_type(node_type)
                node_type.set_has_state(True)

        @staticmethod
        def on_connection_type_resolve(node):
            on_connection_type_resolve_function = getattr(OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS, "on_connection_type_resolve", None)
    ...
    def register(node_type_class):
        ...
        OgnPegasusArduPilotMultirotorNodeDatabase.NODE_TYPE_CLASS = node_type_class
        og.register_node_type(OgnPegasusArduPilotMultirotorNodeDatabase.abi, 2)

    @staticmethod
    def deregister():
        og.deregister_node_type("airlab.pegasus.PegasusArduPilotMultirotorNode")
