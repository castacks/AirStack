...
ASCENTNODE_OPT_IN_SETTING = "/app/airlab.pegasus/opt_in"
ASCENTNODE_ENABLE_OPT_IN_SETTING = "/app/airlab.pegasus/enable_opt_in"
...
def check_for_ascentnodes():
    """
    Check for presence of airlab.pegasus instances and confirm user wants to enable them.
    """
    ...
    for graph in graphs:
        for node in graph.get_nodes():
            ...
            if node_type.get_node_type() == "airlab.pegasus.AscentNode":
                # Found one
...
class _PublicExtension(omni.ext.IExt):
    ...
    def init_physics(self, was_playing=False):
        ...
        async def init_physics():
            if self.world._physics_context == None:
                ...
                if was_playing:
                    self.world.play()
            self.world.add_physics_callback('pegasus_physics_callback', self.physics_callback)
    ...
    def on_shutdown(self):
        ...
        self.__stage_subscription = None
        self.__opt_in_setting_sub = None
        self.world.remove_physics_callback('pegasus_physics_callback')
