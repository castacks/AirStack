"""asset_packs — read a `config/asset_packs/*.yaml` pack into plain asset entries.

Wraps the scene generator's own `resolve_asset_pack` / `_parse_usd_entry`, so
...
def read_asset_pack(name, categories, target_size=0.0):
    ...
    """
    from scene_generator import _parse_usd_entry, resolve_asset_pack

    cfg = resolve_asset_pack({"asset_pack": name})
...
def run_isolated(fn, args):
    ...
    Both `damage_spread.py` and `categorize_assets.py` need to resolve an asset
    ...
    dance, written once. It matters because `read_asset_pack` reaches into
