...
# ─── 0.5. /dev/shm sizing ──────────────────────────────────────────────────
#
# K8s gives the pod a 64Mi /dev/shm, and the inner dockerd on this setup
# shares it with every container while silently ignoring per-container
# shm_size (observed 2026-07-26: HostConfig.ShmSize=4G but a 64M mount, and
# Isaac's carb-RStringInternals-* segments visible from the robot container).
# Once stale carb/fastdds segments from dead processes fill it, Isaac aborts
# at boot ("Error while opening shared memory") and Fast DDS SHM fails. The
# pod is privileged, so remount it big and scrub stale segments every boot.
if mount -o remount,size=8g /dev/shm 2>/dev/null; then
  log "/dev/shm remounted to 8g"
else
  log "WARN: /dev/shm remount failed — Isaac may hit shm exhaustion"
fi
...
# Persist the fully-computed stack env (scene vars, AUTOLAUNCH, NUM_ROBOTS,
# COMPOSE_PROFILES, ...) for interactive SSH shells: a bare `ssh` login has
# NONE of the workflow env, so a manual `airstack down && airstack up` from
# it silently reverts to .env defaults (wrong scene, base isaac-sim profile
# instead of livestream — observed 2026-07-26).
{
  echo "# generated by osmo workspace entrypoint.sh — stack env for ssh shells"
  for v in AUTOLAUNCH NUM_ROBOTS ISAAC_SIM_LIVESTREAM COMPOSE_PROFILES \
           ENV_URL SPAWN_CONFIGS SPAWN_POLY STAGE_SCALE SPAWN_HEIGHT_M \
           RESULTS_SCENE LLM_NAV FRONTIER_ONLY_BASELINE LVLM_BASELINE \
           ISAAC_SIM_SCRIPT_NAME; do
    val="$(printenv "$v" 2>/dev/null)" || continue
    [ -n "$val" ] && printf 'export %s=%q\n' "$v" "$val"
  done
} > /etc/profile.d/airstack-env.sh 2>/dev/null || \
  log "WARN: could not write /etc/profile.d/airstack-env.sh"
