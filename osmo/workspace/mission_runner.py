        ...
        if via == "gcs":
            ...
            def send(n):
                ...
                script = (
                    ...
                    f"fb_sub=$!\n"
                    f"sleep 3\n"
                    f"ros2 topic pub {base}/goal std_msgs/msg/String "
                    f"{shlex.quote(msg_yaml)} --rate 10 --times 10\n"
                    f"sent=$(date +%s)\n"
                    f"while true; do\n"
                    f"  if grep -q '\"success\": true' {result_file}; then\n"
                    f"    grep '\"success\": true' {result_file} | tail -n 1\n"
                    f"    exit 0\n"
                    f"  fi\n"
                    f"  if [ $(( $(date +%s) - sent )) -ge {int(timeout)} ]; then\n"
                    f"    echo 'no success result within {int(timeout)}s'\n"
