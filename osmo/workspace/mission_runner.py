...
Artifacts land under one results root per mission run:
    ...
    └── iter_001__<env>__<method>/   # e.g. iter_001__warehouse__frontier
...
# `method:` is an optional free-form label (on an environment entry or at the
# mission top level) for missions that compare algorithms/configurations across
# iterations — it lands in the iteration dir name and summary.json so runs stay
# distinguishable after fetch.
...
def resolve_method(mission, env_entry):
    """Normalized method label. Explicit `method:` on the environment entry
    wins, then a mission-level `method:`; 'na' (omitted from names) otherwise."""
...
def iter_dir_name(i, env_entry, method):
    ...
    the env name is dropped to avoid e.g. scene_a_frontier__frontier."""
    parts = [f"iter_{i:03d}"]
    env_slug = _slug(env_entry["name"]) if env_entry and env_entry.get("name") else ""
    if env_slug:
        for sep in ("_", "-"):
            if method != "na" and env_slug.endswith(f"{sep}{method}"):
                env_slug = env_slug[: -len(method) - 1]
                break
        if env_slug:
...
# (not /tmp), so they're collected separately — they're what reveals why a
# planner's plan isn't becoming motion (e.g. a robot stuck mid-mission). Task
# nodes are here too so their rcl log is captured as a SECOND path independent
# of the /tmp tee (TEE_LOG_GLOBS) — if one collection path misses, the other
# still shows why the drone wasn't commanded.
...
# /tmp tee logs written by task-spawned subprocesses and the GCS
# relay/gossip/ddsrouter — the stdout that rcl logging doesn't capture.
TEE_LOG_GLOBS = ("/tmp/gossip_*.log", "/tmp/ddsrouter_*.log", "/tmp/relay_*.log")
...
def _copy_tmp_tee_logs(dest_dir):
    ...
    leaves the most recent stdout tees on disk."""
...
def snapshot_task_logs(dest_dir):
    """Copy the raw gossip/ddsrouter/relay tees (written to /tmp inside each
    container) into <dest_dir>/<container>/, plus the navigation /
...
# full command line (python nodes all run as `python3 ...`). Extend per-mission
# with any long-running node whose silent crash would otherwise only show up as
# a stuck drone.
_WATCH_PROCS = ("ddsrouter", "gossip")
...
def _format_resources(mem, ps_rows):
    ...
    for rss, args in ps_rows:
        for k in _WATCH_PROCS:
            if k in args:
                key[k] += rss
    keystr = " ".join(f"{k}={v // 1024}MB" for k, v in key.items())
    top = []
...
def transport_snapshot_loop(iter_dir, stop_event, interval_s=5):
    ...
    # stdout tees. ~every 30s regardless of the health interval.
