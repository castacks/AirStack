...
## Flying a FROZEN dataset cell instead of building a scene

`FROZEN_SCENE` in a mission's `env:` (or, per-iteration, in an `environments:`
entry) points `example_multi_drone_scene_import.py` at one of the finished
`.usd` files under `final_disaster_dataset/` instead of building a plat with
`SCENE_CONFIG`. The cell is bind-mounted into the isaac-sim container at
`/isaac-sim/final_disaster_dataset` (host side: `FINAL_DATASET_DIR` in `.env`),
and any of three spellings resolves:

```yaml
FROZEN_SCENE: Fire/Suburban/level_1/1                          # the cell
FROZEN_SCENE: Fire/Suburban/level_1/1/fire_suburban_lvl1_1.usd # under the mount
FROZEN_SCENE: /isaac-sim/final_disaster_dataset/.../x.usd      # absolute
```

Two consequences for a mission spec:

* **`ready.timeout_s` comes down.** A frozen cell is a file open, not a
  20-minute procedural assembly — 2400 s covers the reference plus eight PX4
  airframes, against the built path's 4200.
* **`GT_ANNOTATIONS: "on"` writes DIFFERENT files.** The launcher reshapes the
  cell's own `GT_people.json` / `GT_hints.json` rather than measuring the
  stage, and the region file's search-area key is `search` (not the fire
...
x 8 robots, 24 iterations, `environments:` generated by
`scene_gen/tools/frozen_cell_plan.py`. Background:
`.agents/skills/benchmark-disaster-dataset/SKILL.md` section 2b.
