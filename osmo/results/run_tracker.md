...
| 8b | `airstack-mission-1gpu-58` / dev 191 | Earthquake/Suburban L2 VLFM + CoNavGPT2 replacement | 0/2 | 12 h mission / 100 h inspectable pod | PENDING/SCHEDULING — workflow 58 was submitted after pod 56 expired. In parallel, dev 191 is running the upload-disabled corrected 100-s VLFM gate on its reserved UUID (`GPU-38264ce2…`, Isaac index 3). Promotion requires GPU solver/direct-API, all-eight flight/perception and timed RTF ≥0.12; the workflow mission's renderer index must be corrected after its own pod assignment is known |
