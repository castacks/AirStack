...
| 2-GPU 3 | Fire Suburban L1 and L3 RAVEN are both **passed, uploaded and NAS-verified**; Hurricane Suburban L2 transport-fixed retry started at 02:24:08 UTC | The prior retry's reliable-only post-takeoff gate rejected routed odometry on varying robots. Live probes proved their advancing converted odometry remained available from the DDS router as BEST_EFFORT. Commit `4c846087` makes both the task clock and gate compatible with reliable local plus best-effort routed publishers while retaining the 30 s stale-stream abort. All rejected attempts remain local and unuploaded. Previous live RTF was **0.10740**. Launcher PID 60 remains T |
