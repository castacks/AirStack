...
| 2-GPU 3 | Fire Suburban L1 and L3 RAVEN are both **passed, uploaded and NAS-verified**; corrected Hurricane Suburban L2 retry started at **03:09:11 UTC** | The 02:24 retry exposed a mission-runner false failure: an accepted takeoff could take several wall minutes at low RTF, but the generic 15 s feedback timeout resent the goal; the task correctly rejected the duplicate while the original later succeeded. Commit `3e27df89` gives takeoff 300 s for real feedback/result, retaining the BEST_EFFORT routed-odometry compatibility, 30 s stale-stream abort and 8×600 s result gate. All rejected attempts remain local and unuploaded. Launcher PID 60 remains T |
...
| 2-GPU 5 | Protected for inspection; stale Urban Fire L1/L2 standard-baseline runner has **0 passed / 5 failed attempts** and is currently repeating the same preflight gate | Every failure is the same configuration-contract defect: the mission requested `mono_rgbd`, but `ZED_PIPELINE` was absent inside robot containers, so the gate incorrectly waited for stereo disparity that cannot exist. Launcher PID 61 is T; runner PID 12068 is preserved pending safe RAVEN repurpose authorization. GPUs are 2× RTX PRO 5000 Blackwell; Isaac uses inner 0 while inner 1 is idle. No failed attempt uploaded. |
