...
NAS. L2 Frontier started next at **07:33:34 UTC**.
...
| 10 | dev pod 191 | Hurricane/Urban L1 × Frontier, lawnmower, VLFM, CoNavGPT2 | 1/4 | ≤11 h | RUNNING — Frontier **passed** 17/17 steps at bottleneck/team RTF **0.17664** (600.03 sim s / 3396.96 wall s), then its 3.9 GiB result was uploaded and NAS-verified under `hurricane_urban_l1_8robot_optimized_dev191/2026-09-07_04-58-24/iter_001__hurricaneurbanl1v1__frontier`; PX4, Mighty bridge, perception, and takeoff gates passed for all 8 robots after one successful MAVROS recovery. Lawnmower started at 06:17 UTC and is active. The launcher's storage env expanded an unquoted `$` in the password, so passed iterations are being manually uploaded with literal credential parsing and remote `iteration.json`/MCAP verification |
