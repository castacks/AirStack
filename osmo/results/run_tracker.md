...
| 2-GPU 3 | Fire Suburban L1 and L3 RAVEN are both **passed, uploaded and NAS-verified**; Hurricane Suburban L2 entered 8-robot search at **03:35:31 UTC** | The corrected retry passed 8/8 PX4, perception, physical takeoff and advancing-odometry gates. Persistent callback measurement is **RTF 0.09226** over 90.07 wall s / 8.31 sim s. Commit `3e27df89` prevents premature duplicate takeoff retries while retaining the 30 s stale-stream abort and 8×600 s result gate. All rejected attempts remain local and unuploaded. Launcher PID 60 remains T |
...
| 2-GPU 5 | Hurricane Suburban L3 RAVEN replacement entered 8-robot search at **03:54:26 UTC** on renderer GPU 0 / offboard GPU 1 | Attempt 1 correctly failed the encoder gate because this stale pod lacked the offline SigLIP2 cache; it was not uploaded. After an exact cache transfer, attempt 2 passed the live CUDA encoder, 8-robot frame, PX4, perception, takeoff and advancing-odometry gates. Persistent callback measurement is **RTF 0.10253** over 90.12 wall s / 9.24 sim s. It retains the 8×600 s result gate and checksum-verified upload/prune. Launcher PID 61 remains T. |
