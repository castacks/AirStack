...
from mtl_metrics_logger.detection import DetectionModel, TeamScorer, resample_hold
from mtl_metrics_logger.report import build_report_data, write_json, write_report, write_telemetry_csv
...
def score_agents(rows_by_agent: Mapping[str, Sequence[Mapping[str, Any]]], scenario: Mapping[str, Any],
                 ...
                 dt: float | None = None) -> tuple[TeamScorer, dict[str, dict[str, list]], dict[str, float]]:
    ...
    scorer = TeamScorer(targets_world(ground_truth), cells, masses, model, fov)
...
def write_run_outputs(out_dir: str | Path, *, scenario: Mapping[str, Any], ground_truth: Mapping[str, Any],
    ...
    """Score and write ``telemetry.csv``, ``detection.json`` and ``report.html``.

    ``planned_by_agent[name]``: ``{"planned": [[x, y]...] (world), "home": [x, y],
    "serviced_cells": [...], "planned_length_m": ...}``.
    """
    out = Path(out_dir)
    out.mkdir(parents=True, exist_ok=True)
    scorer, per_agent, measured = score_agents(rows_by_agent, scenario, ground_truth)
    ...
    detection = {
        ...
        "curves": {"t_s": [round(v, 3) for v in scorer.t], "targets_detected": scorer.detected_count,
                   ...
                   "covered_mass": [round(v, 3) for v in scorer.mass_curve]},
