...
namespace bpmp_tracker{
    ...
    class BPMPTracker : public rclcpp::Node{
        ...
        rclcpp::Client<airstack_msgs::srv::TrajectoryMode>::SharedPtr set_trajectory_mode_client_;

        // Bool topic subscriber (from Foxglove) — mirrors keyboard_control_enable pattern
        rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr tracking_enable_sub_;
        void TrackingEnableCallback(const std_msgs::msg::Bool::SharedPtr msg);
        void ApplyTrackingEnabled(bool enabled);

        // Service server for target_tracking_toggle (CLI / programmatic use)
