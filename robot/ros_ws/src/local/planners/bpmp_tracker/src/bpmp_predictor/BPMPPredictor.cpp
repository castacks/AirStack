...
namespace bpmp_tracker{
        ...
        const sensor_msgs::msg::Image::ConstSharedPtr depth_msg)
    {
        ...
        // currently visible, hold this cycle — do NOT switch to a different
        // person. Publishing nothing leaves the tracker on the last known
        // target rather than hijacking onto whoever else is in view.
        const humanflow_msgs::msg::HumanFlow* target_human =
            find_human_by_global_id(target_id_);
        if (target_human == nullptr) {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                "Target global ID %d not currently visible in ReID mapping "
                "(mapping size: %zu); holding target",
