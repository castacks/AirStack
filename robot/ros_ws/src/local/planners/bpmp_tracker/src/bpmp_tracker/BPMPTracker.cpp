...
namespace bpmp_tracker{
    ...
    void BPMPTracker::initialize(){
        ...
        desired_traj_publisher_ = this->create_publisher<airstack_msgs::msg::TrajectoryXYZVYaw>("trajectory_override", 5);

        // Bool topic subscriber — Foxglove publishes here to enable/disable tracking
        tracking_enable_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            "target_tracking_enable", 5,
            std::bind(&BPMPTracker::TrackingEnableCallback, this, std::placeholders::_1));

        // SERVICE CLIENT for trajectory mode switching
        set_trajectory_mode_client_ = this->create_client<airstack_msgs::srv::TrajectoryMode>("set_trajectory_mode");

        // SERVICE SERVER for target_tracking_toggle
    ...
    void BPMPTracker::ApplyTrackingEnabled(bool enabled) {
        ...
        if (set_trajectory_mode_client_->service_is_ready()) {
            ...
            req->mode = tracking_enabled_
                ? airstack_msgs::srv::TrajectoryMode::Request::TRACK
                : airstack_msgs::srv::TrajectoryMode::Request::ROBOT_POSE;
            set_trajectory_mode_client_->async_send_request(req);
        }

        RCLCPP_INFO(this->get_logger(), "Target tracking %s", tracking_enabled_ ? "ENABLED" : "DISABLED");
    }

    void BPMPTracker::TrackingEnableCallback(const std_msgs::msg::Bool::SharedPtr msg) {
        ApplyTrackingEnabled(msg->data);
    }
    ...
    {
        ApplyTrackingEnabled(!tracking_enabled_);
        response->success = true;
        response->message = tracking_enabled_ ? "Target tracking enabled" : "Target tracking disabled";
    }
