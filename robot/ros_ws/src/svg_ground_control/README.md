...
- **Runtime tuning**: `cbf_alpha` can be changed while flying and applies on
  the next control tick (`ros2 param set /swarm_commander cbf_alpha 4.0`, or
  the panel's CBF slider via `set_parameters`); non-positive / non-finite
  values are rejected. Other parameters are read once at startup.
