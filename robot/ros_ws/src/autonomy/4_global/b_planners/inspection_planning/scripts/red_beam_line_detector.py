...
class RedBeamLineDetector(Node):
    def __init__(self):
        ...
        # Data storage
        self.vdb_voxels = []
        self.detected_lines = []  # List of detected lines
        self.next_line_id = 0  # Counter for unique line IDs
        self.voxel_size = 0.2
        ...
        self.min_points_per_voxel = 2
        self.min_beam_length = 8.0  # Minimum length for a valid beam
        self.beam_thickness_threshold = 0.5  # Maximum width for a valid beam
        ...
        # New publisher for panoptic-style markers
        self.pub_panoptic_markers = self.create_publisher(
            MarkerArray,
            '/panoptic/markers',
            10
        )
    ...
    def point_cloud_callback(self, msg):
        ...
        self.publish_panoptic_markers()  # Add this line
    ...
    def update_detected_lines(self, new_lines):
        """Update detected lines list, removing duplicates and keeping longer lines with unique IDs"""
        for new_line in new_lines:
            # Check if this line is close to any existing line
            is_duplicate = False
            lines_to_remove = []
            ...
            for i, existing_line in enumerate(self.detected_lines):
                if self.are_lines_similar(new_line, existing_line):
                    is_duplicate = True
                    # Keep the longer line and transfer the ID
                    if new_line['length'] > existing_line['length']:
                        # Transfer the existing ID to the new line
                        new_line['unique_id'] = existing_line['unique_id']
                        lines_to_remove.append(i)
                        break
                    else:
                        # New line is shorter, don't add it
                        break
            ...
            # Add new line if it's not a duplicate or if it's longer than existing ones
            if not is_duplicate or (is_duplicate and new_line['length'] > existing_line['length']):
                if 'unique_id' not in new_line:
                    # Assign new unique ID
                    new_line['unique_id'] = self.next_line_id
                    self.next_line_id += 1
    ...
    def publish_panoptic_markers(self):
        """Publish markers in panoptic format with unique IDs"""
        ...
        for i, line in enumerate(self.detected_lines):
            ...
            # Principal axis is the line direction
            principal_axis = np.array(line['direction'])
            ...
            text_marker.text = f"{line['unique_id']}_#10"
            ...
            # Set arrow end point (centroid + principal_axis * 0.4)
            end_point = geometry_msgs.msg.Point()
        ...
        # Publish results
        if markers.markers:
            self.pub_panoptic_markers.publish(markers)
