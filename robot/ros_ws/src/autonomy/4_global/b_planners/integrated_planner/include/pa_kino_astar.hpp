...
private:
    ...
    double safe_robot_r_, safe_index_dist_, safe_sq_index_dist_;
...
public:
    ...
    /* main API */
    void setParam(std::shared_ptr<rclcpp::Node> node);
    void init(const std::shared_ptr<VDBMap> &map, double safe_robot_r);
    void reset();
    ...
    void setEnvironment(const std::shared_ptr<VDBMap> &map);

    void publishDebugVis();

    // xyz, vxyz, axyz
    std::vector<Eigen::Matrix<double, 9, 1>> getKinoTraj(double delta_t);
