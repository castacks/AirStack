...
bool RRT_Planner::validEdge(const ViewPoint &p_new, const ViewPoint &p_near, collision_checker_ns::CollisionChecker &collision_checker)
{
    ...
    if (diff < step_size)
    {
        if (collision_checker.collisionCheckInFreeSpace(p_new) && collision_checker.collisionCheckInFreeSpace(p_near))
        {
            return false;
        }
        else
    ...
    ViewPoint step = multiply(direction, step_size);
...
bool RRT_Planner::validEdgeLoose(const ViewPoint &p_new, const ViewPoint &p_near, collision_checker_ns::CollisionChecker &collision_checker)
{
    // Collision checking at discrete points between the two points?
    ...
    direction = divide(direction, diff);
    // ViewPoint step = multiply(direction, double(kNorm_limit_)); // kVoxel_dim_
    ViewPoint step = multiply(direction, double(0.2));
    int num_steps = ceil(diff / kNorm_limit_); // kVoxel_dim_
    ...
    {
        check_point = addPoint(check_point, step);
        if (collision_checker.collisionCheckInFreeSpaceShrink(check_point))
        {
            return false;
        }
    }
...
ViewPoint RRT_Planner::getEdgeColl(const ViewPoint &p_new, const ViewPoint &p_near, collision_checker_ns::CollisionChecker &collision_checker)
{
    // Collision checking at discrete points between the two points?
...
//     if (collision_checker.collisionCheckRayLoose(p_new, p_near, hit_point))
//     {
//         return false;
//     }
//     return true;
...
PointSet RRT_Planner::getExecutingPath()
{
    return executing_path_;
}
                             ...
                             collision_checker_ns::CollisionChecker &collision_checker)
{
    ...
    executing_path_.push_back(anchor);
    std::cout << "init add waypoint " << executing_path_.size() - 1 << " at " << 0 <<" position " << anchor.x << ", " << anchor.y << ", " << anchor.z << '\n';
    ...
    for (size_t i = 1; i < Pr.size(); ++i)
    {
        ...
        // const bool blocked = collision_checker.collisionCheckRayLoose(anchor, Pr[i], hit_world);
        bool unblocked = validEdge(anchor, Pr[i], collision_checker);
        ...
        if (unblocked)
        {
            ...
            if (i == Pr.size() - 1)
            {
                ...
                path.push_back(Pr[i]);
                executing_path_.push_back(Pr[i]);
                std::cout << "case0 add waypoint " << executing_path_.size() - 1 << " at " << i <<" position " << Pr[i].x << ", " << Pr[i].y << ", " << Pr[i].z << '\n';
                std::cout << "add valid? " << validEdge(executing_path_[executing_path_.size() - 2], executing_path_[executing_path_.size() - 1], collision_checker) <<"\n";
                        
            }
            continue;
        }

        hit_world = point2vdb(getEdgeColl(anchor, Pr[i], collision_checker));
            ...
            for (int step_count=0; step_count < max_pivot_steps; step_count++)
            {
                ...
                if (!collision_checker.collisionCheckInFreeSpace(pivot))
                {
                        ...
                        validEdge(pivot, Pr[i], collision_checker))
                    {
                        ...
                        path.push_back(anchor);
                        executing_path_.push_back(anchor);
                        std::cout << "case1 add waypoint " << executing_path_.size() - 1 << " at " << i << " position " << anchor.x << ", " << anchor.y << ", " << anchor.z << '\n';
                        std::cout << "add valid? " << validEdge(executing_path_[executing_path_.size() - 2], executing_path_[executing_path_.size() - 1], collision_checker) <<"\n";
                        if (i == Pr.size() - 1)
                        {
                            interpolate(anchor, Pr[i], path);
                            path.push_back(Pr[i]);
                            executing_path_.push_back(Pr[i]);
                            std::cout << "case2 add waypoint " << executing_path_.size() - 1 << " at " << i << " position " << Pr[i].x << ", " << Pr[i].y << ", " << Pr[i].z << '\n';
                            std::cout << "add valid? " << validEdge(executing_path_[executing_path_.size() - 2], executing_path_[executing_path_.size() - 1], collision_checker) <<"\n";
                        }
                        break;
                    }
        ...
        if (!pivot_inserted)
        {
            ...
            path.push_back(anchor);
            executing_path_.push_back(anchor);
            std::cout << "case3 add waypoint " << executing_path_.size() - 1 << " at " << i << " position " << anchor.x << ", " << anchor.y << ", " << anchor.z << '\n';
            std::cout << "add valid? " << validEdge(executing_path_[executing_path_.size() - 2], executing_path_[executing_path_.size() - 1], collision_checker) <<"\n";
            if (!validEdge(executing_path_[executing_path_.size() - 2], executing_path_[executing_path_.size() - 1], collision_checker))
            {
                std::cout << "checking point " << executing_path_.size() - 2 << ": " << collision_checker.collisionCheckInFreeSpace(executing_path_[executing_path_.size() - 2]);
                std::cout << "checking point " << executing_path_.size() - 1 << ": " << collision_checker.collisionCheckInFreeSpace(executing_path_[executing_path_.size() - 1]);
            }
            if (i == Pr.size() - 1)
            {
                path.push_back(Pr[i]);
                executing_path_.push_back(Pr[i]);
                std::cout << "case4 add waypoint " << executing_path_.size() - 1 << " at " << i << " position " << Pr[i].x << ", " << Pr[i].y << ", " << Pr[i].z << '\n';
                std::cout << "add valid? " << validEdge(executing_path_[executing_path_.size() - 2], executing_path_[executing_path_.size() - 1], collision_checker) <<"\n";
            }
        }
    }

    for (size_t i = 1; i < executing_path_.size(); ++i)
    {
        if(!validEdge(executing_path_[i-1], executing_path_[i], collision_checker))
        {
            std::cout << "hit waypoint edge end index is " << i << '\n';
            std::cout << "hit waypoint edge is " << executing_path_[i-1].x << ", " << executing_path_[i-1].y << ", " << executing_path_[i-1].z << ", " << "\n to "
                                                << executing_path_[i].x << ", " << executing_path_[i].y << ", " << executing_path_[i].z << "\n";
            std::cout << "build valid? " << validEdge(executing_path_[i-1], executing_path_[i], collision_checker) <<"\n";
            return;
        }
    }
...
bool RRT_Planner::executingPathCollision(collision_checker_ns::CollisionChecker &collision_checker)
{
    ...
    for (size_t i = 1; i < executing_path_.size(); ++i)
    {
        if(!validEdgeLoose(executing_path_[i-1], executing_path_[i], collision_checker))
        {
            std::cout << "hit waypoint edge end index is " << i << '\n';
            std::cout << "hit waypoint edge is" << executing_path_[i-1].x << ", " << executing_path_[i-1].y << ", " << executing_path_[i-1].z << ", " << "\n to"
                                                << executing_path_[i].x << ", " << executing_path_[i].y << ", " << executing_path_[i].z << "\n";
            std::cout << "check valid?" << validEdgeLoose(executing_path_[i-1], executing_path_[i], collision_checker) <<"\n";
            return true;
        }
    }
    return false;
...
ViewPoint RRT_Planner::getPathCollision(collision_checker_ns::CollisionChecker &collision_checker)
{
    ...
    for (size_t i = 1; i < executing_path_.size(); ++i)
    {
        if(!validEdge(executing_path_[i-1], executing_path_[i], collision_checker))
        {
            return getEdgeColl(executing_path_[i-1], executing_path_[i], collision_checker);
        }
