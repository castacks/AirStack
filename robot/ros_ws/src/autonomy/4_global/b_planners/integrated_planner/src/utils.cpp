                                     ...
                                     bool consider_unknown_occupied)
{
    ...
    // Validate transformed points (prevents NaN maxTime in DDA)
    auto finite3 = [](const openvdb::Vec3d &v)
    {
        return std::isfinite(v.x()) && std::isfinite(v.y()) && std::isfinite(v.z());
    };
    if (!finite3(p1_ijk) || !finite3(p2_ijk))
    {
        return false; // or log & early-out
    }

    openvdb::Vec3d dir(p2_ijk - p1_ijk);
    double distance = dir.length();

    // Guard zero/near-zero or non-finite distance (prevents bad ray)
    if (!(distance > 1e-9) || !std::isfinite(distance))
    {
        return false; // nothing to traverse (or invalid)
    }
