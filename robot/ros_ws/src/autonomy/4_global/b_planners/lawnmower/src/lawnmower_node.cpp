#include "../include/lawnmower_node.hpp"

#include "../include/lawnmower_logic.hpp"

std::optional<init_params> LawnmowerNode::readParameters(){
    init_params params;
    ...
    this->declare_parameter<std::string>("srv_lawnmower_toggle_topic");
    if (!this->get_parameter("srv_lawnmower_toggle_topic", this->srv_lawnmower_toggle_topic_)) {
        RCLCPP_ERROR(this->get_logger(), "Cannot read parameter: srv_lawnmower_toggle_topic");
        return std::optional<init_params>{};
    }
...
LawnmowerNode::LawnmowerNode() : Node("lawnmower_node") {
    //initialize lawnmower planner
    std::optional<init_params> params_opt = LawnmowerNode::readParameters();
    if (params_opt.has_value()){
        this->params = params_opt.value();
    } else {
        RCLCPP_ERROR(this->get_logger(), "Failed to initialize Lawnmower planner");
    }
    
    this->sub_map = this->create_subscription<visualization_msgs::msg::Marker>(
        sub_map_topic_, 10, std::bind(&LawnmowerNode::mapCallback, this, std::placeholders::_1));
    ...
    // set up the timer
    this->timer = this->create_wall_timer(std::chrono::seconds(1),std::bind(&LawnmowerNode::timerCallback,this));

    // Set up the service
    this->srv_lawnmower_toggle = this->create_service<std_srvs::srv::Trigger>(
        srv_lawnmower_toggle_topic_, std::bind(&LawnmowerNode::lawnmowerToggleCallback, this,
                                                 std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO(this->get_logger(), "lawnmower node initialized");
}

void LawnmowerNode::lawnmowerToggleCallback(
    const std_srvs::srv::Trigger::Request::SharedPtr request,
    std_srvs::srv::Trigger::Response::SharedPtr response) {
    if (this->enable_lawnmower == false) {
        this->enable_lawnmower = true;
        response->success = true;
        response->message = "lawnmower enabled";
        RCLCPP_INFO(this->get_logger(), "lawnmower enabled");
    } else {
        this->enable_lawnmower = false;
        response->success = true;
        response->message = "lawnmower disabled";
        RCLCPP_INFO(this->get_logger(), "lawnmower disabled");
    }
}

void LawnmowerNode::mapCallback(const visualization_msgs::msg::Marker::SharedPtr msg){
    if (!this->received_first_map){
        ...
        this->lawnmower_planner = std::make_unique<LawnmowerPlanner>(this->params);
        RCLCPP_INFO(this->get_logger(), "Initialized lawnmower planner logic");
    }
    this->lawnmower_planner->voxel_points.clear();
    for (int i=0; i< msg->points.size(); i++){
        this->lawnmower_planner->voxel_points.push_back(
            std::tuple<float,float,float>(msg->points[i].x, msg->points[i].y, msg->points[i].z));
    }
}

void LawnmowerNode::timerCallback(){
    try {
    ...
    if (this->enable_lawnmower && !this->is_path_executing) {
    ...
    } else if (this->enable_lawnmower && this->is_path_executing) {
...
void LawnmowerNode::tfCallback(const tf2_msgs::msg::TFMessage::SharedPtr msg) {
...
void LawnmowerNode::generate_plan(){
    ...
    std::optional<Path> gen_path_opt =
        this->lawnmower_planner->generate_straight_rand_path(start_loc, timeout_duration);
...
void LawnmowerNode::publish_plan() {
...
int main(int argc, char *argv[]){
    ...
    rclcpp::spin(std::make_shared<LawnmowerNode>());
    rclcpp::shutdown();
    return 0;
}