...
LawnmowerPlanner::LawnmowerPlanner(init_params params)
{
...
std::optional<Path> LawnmowerPlanner::generate_straight_rand_path(
    std::tuple<float, float, float, float> start_point, float timeout_duration)
{
                                           ...
                                           .count() < timeout_duration)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("lawnmower_planner"),
        ...
        while (get_point_distance(current_point, goal_point) > 0.001)
        {
            ...
            {
                RCLCPP_INFO(rclcpp::get_logger("lawnmower_planner"), "Collision detected");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("lawnmower_planner"), "No Path found in time limit");
        return std::nullopt;
    }
    else
    {
...
bool LawnmowerPlanner::check_if_collided_single_voxel(
...
bool LawnmowerPlanner::check_if_collided(const std::tuple<float, float, float>& point)
{
    ...
    for (const std::tuple<float, float, float>& voxel : this->voxel_points)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("lawnmower_planner"), "Voxel is 0,0,0");
        }
...
std::tuple<float, float, float> LawnmowerPlanner::generate_goal_point(
                          ...
                          const std::tuple<float, float, float>& point2)
{
    ...
    if (std::isnan(dist) || std::isinf(dist)) {
        RCLCPP_ERROR(rclcpp::get_logger("lawnmower_planner"), "Distance is nan or inf");
    }
    return dist;
}
