...
public:
  OdometryConversion()
    ...
    odometry_output_type = (OdometryOutputType)airstack::get_param(this, "odometry_output_type", (int)NONE);
    convert_odometry_to_transform = airstack::get_param(this, "convert_odometry_to_transform", false);
    convert_odometry_to_stabilized_transform = airstack::get_param(this, "convert_odometry_to_stabilized_transform", false);
    ...
    odom_orientation_rotation_pre = tf2::Quaternion(airstack::get_param(this, "odometry_orientation_rotation_pre_x", 0.),
    ...
    odom_position_translation_pre = tf2::Vector3(airstack::get_param(this, "odometry_position_translation_pre_x", 0.),
    ...
    odom_orientation_rotation_post = tf2::Quaternion(airstack::get_param(this, "odometry_orientation_rotation_post_x", 0.),
    ...
    odom_position_translation_post = tf2::Vector3(airstack::get_param(this, "odometry_position_translation_post_x", 0.),
    ...
    odom_sub = this->create_subscription<nav_msgs::msg::Odometry>("odometry_in", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(qos),
											     qos),
								  std::bind(&OdometryConversion::odom_callback,
  ...
  void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg){
    ...
    if(restamp_now_pre)
      out_odom.header.stamp = get_clock()->now();
    
    out_odom.pose.pose.orientation = tflib::from_tf(tflib::to_tf(out_odom.pose.pose.orientation)*odom_orientation_rotation_pre);
    ...
    if(odometry_output_type == NONE){
      ...
      try{
	out_odom = tflib::transform_odometry(tf_buffer, out_odom, new_frame_id, new_child_frame_id);
      }
      catch(tf2::TransformException& ex){
	RCLCPP_ERROR_STREAM(get_logger(), "TransformException while transforming odometry: " << ex.what());
	return;
      }
    }
      ...
      RCLCPP_ERROR_STREAM(get_logger(), "Unsupported odometry output type: " << odometry_output_type);
    }
    
    if(restamp_now_post)
      out_odom.header.stamp = get_clock()->now();
    
    out_odom.pose.pose.orientation = tflib::from_tf(tflib::to_tf(out_odom.pose.pose.orientation)*odom_orientation_rotation_post);
    ...
    if(convert_odometry_to_stabilized_transform){
      ...
      t.transform.rotation = tflib::from_tf(tflib::get_stabilized(tflib::to_tf(out_odom.pose.pose.orientation)));
      tf_broadcaster->sendTransform(t);
    }
