...
def get_accelerations(traj):
    ...
    for i in range(len(traj.waypoints)-2):
        ...
        if abs(dx1-0)<1e-6:
            ...
            traj.waypoints[i].acceleration.x = min((v_nextx**2 - v_currx**2)/(2*dx1), acc_limit)

        if abs(dy1-0)<1e-6:            
            traj.waypoints[i].acceleration.y = 0.
        else:            
            traj.waypoints[i].acceleration.y = min((v_nexty**2 - v_curry**2)/(2*dy1), acc_limit)
...
class FixedTrajectoryGenerator(Node):
    def __init__(self):
        ...
        self.fixed_trajectory_sub = self.create_subscription(FixedTrajectory, 'fixed_trajectory_command', self.fixed_trajectory_callback, 1)
        self.trajectory_track_pub = self.create_publisher(TrajectoryXYZVYaw, 'trajectory_track', 1)
