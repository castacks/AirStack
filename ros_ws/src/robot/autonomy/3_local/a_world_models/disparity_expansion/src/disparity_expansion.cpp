...
void DisparityExpansionNode::getCamInfo(
    ...
    this->cx_ = model_.cx() / downsample_scale;
    this->cy_ = model_.cy() / downsample_scale;
    this->fx_ = fy_ = model_.fx() / downsample_scale;
    this->width = msg_info->width / downsample_scale;
    this->height = msg_info->height / downsample_scale;
    ...
    if (this->baseline == 0.0) {
        ...
        RCLCPP_ERROR_STREAM_ONCE(
            this->get_logger(),
            "Baseline from camera_info was 0, setting to baseline_fallback:" << this->baseline);
    }
    this->baseline *= downsample_scale;
