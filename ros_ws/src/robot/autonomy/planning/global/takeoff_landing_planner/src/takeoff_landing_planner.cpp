...
TakeoffLandingPlanner::TakeoffLandingPlanner()
  : rclcpp::Node("takeoff_landing_planner"){
  ...
  landing_tracking_point_ahead_time = airstack::get_param(this, "landing_tracking_point_ahead_time", 5.0);
  ...
  takeoff_path_relative_to_orientation = airstack::get_param(this, "takeoff_path_relative_to_orientation", false);
  
  // init subscribers
  tf_buffer = new tf2_ros::Buffer(this->get_clock());
  tf_listener = new tf2_ros::TransformListener(*tf_buffer);
  completion_percentage_sub =
    this->create_subscription<std_msgs::msg::Float32>("trajectory_completion_percentage", 1,
						      std::bind(&TakeoffLandingPlanner::completion_percentage_callback,
								this, std::placeholders::_1));
  tracking_point_sub =
    this->create_subscription<airstack_msgs::msg::Odometry>("tracking_point", 1,
							    std::bind(&TakeoffLandingPlanner::tracking_point_callback,
								      this, std::placeholders::_1));
  robot_odom_sub = this->create_subscription<nav_msgs::msg::Odometry>("odometry", 1,
								      std::bind(&TakeoffLandingPlanner::robot_odom_callback,
										this, std::placeholders::_1));
  ekf_active_sub = this->create_subscription<std_msgs::msg::Bool>("ekf_active", 1,
								  std::bind(&TakeoffLandingPlanner::ekf_active_callback,
									    this, std::placeholders::_1));
  high_takeoff_sub = this->create_subscription<std_msgs::msg::Bool>("high_takeoff", 1,
								    std::bind(&TakeoffLandingPlanner::high_takeoff_callback,
									      this, std::placeholders::_1));

  // init publishers
  traj_track_pub = this->create_publisher<airstack_msgs::msg::TrajectoryXYZVYaw>("trajectory_track", 1);
  ...
  // init services
  command_server =
    this->create_service<airstack_msgs::srv::TakeoffLandingCommand>("set_takeoff_landing_command",
								    std::bind(&TakeoffLandingPlanner::set_takeoff_landing_command,
...
void TakeoffLandingPlanner::timer_callback(){
  ...
  if(current_command == airstack_msgs::srv::TakeoffLandingCommand::Request::TAKEOFF){
	...
	geometry_msgs::msg::TransformStamped t = tf_buffer->lookupTransform(tracking_point_odom.header.frame_id,
									    robot_odom.header.frame_id, robot_odom.header.stamp,
									    rclcpp::Duration::from_seconds(0.1));
	tf2::fromMsg(t, transform);
	
	tf2::Vector3 tracking_point_position = tflib::to_tf(tracking_point_odom.pose.position);
	tf2::Vector3 robot_odom_position = transform*tflib::to_tf(robot_odom.pose.pose.position);
	
	distance = tracking_point_position.distance(robot_odom_position);
      }
      catch(const tf2::TransformException& ex){
	RCLCPP_ERROR_STREAM(this->get_logger(), "TransformException in TakeoffMonitor: " << ex.what());
      }
	...
	if((rclcpp::Time(robot_odom.header.stamp) - takeoff_acceptance_start).seconds() >= takeoff_acceptance_time || ekf_active){
  ...
  if(current_command == airstack_msgs::srv::TakeoffLandingCommand::Request::LAND){
    ...
    if(robot_odoms.size() > 2){
      ...
      float time_diff = (rclcpp::Time(robot_odom.header.stamp) - rclcpp::Time(initial_odom.header.stamp)).seconds();
      ...
      while(!robot_odoms.empty() && (rclcpp::Time(robot_odom.header.stamp) -
				     rclcpp::Time(robot_odoms.front().header.stamp)).seconds() > landing_acceptance_time){
	...
	RCLCPP_ERROR_STREAM(this->get_logger(), "TransformException in TakeoffMonitor landing tf lookup: " << ex.what());
      }
      bool tracking_point_check = (z_distance/takeoff_landing_velocity) > landing_tracking_point_ahead_time;
      RCLCPP_INFO_STREAM(this->get_logger(), "landing: " << z_distance << " " << (z_distance/takeoff_landing_velocity)
...
void TakeoffLandingPlanner::set_takeoff_landing_command(const airstack_msgs::srv::TakeoffLandingCommand::Request::SharedPtr request,
