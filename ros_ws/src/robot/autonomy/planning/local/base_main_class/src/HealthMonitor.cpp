...
HealthMonitor::HealthMonitor(Base* base) : base_(base) {}
...
int64_t HealthMonitor::toc(std::string id) {
    ...
    if (times.count(id) > 0) {
        ...
        microseconds delta_us = boost::chrono::duration_cast<microseconds>(
            boost::chrono::system_clock::now() - times[id].start);
        ...
        if (!is_hitting_target_hz)
            ROS_ERROR_STREAM(std::setprecision(2)
                             << "execute function is not running fast enought."
                             << " Actual Hz: " << hz << " Target Hz: " << times[id].target_hz);
...
double HealthMonitor::get_target_hz(std::string id) { return times[id].target_hz; }

void HealthMonitor::print_time_statistics() {
    ROS_INFO_STREAM("Time Statistics for " << base_->get_node_name());  // node_name_);
    int col_width = 15;
    ROS_INFO_STREAM(std::setw(col_width)
                    << "Name" << std::setw(col_width) << "Avg Hz" << std::setw(col_width)
                    << "Target Hz" << std::setw(col_width) << "Avg ms" << std::setw(col_width)
                    << "Min ms" << std::setw(col_width) << "Max ms" << std::setw(col_width)
                    << "Calls");
    ...
    for (std::map<std::string, time_info>::iterator it = times.begin(); it != times.end(); it++) {
        if (it->second.calls == 0) {
            ...
            ROS_INFO_STREAM(std::setprecision(2)
                            << std::setw(col_width) << it->first << std::setw(col_width)
                            << average_hz << std::setw(col_width) << it->second.target_hz
