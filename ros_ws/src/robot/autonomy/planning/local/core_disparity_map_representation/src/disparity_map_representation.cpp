...
/*std::vector< std::vector<double> >
  DisparityMapRepresentation::get_values(std::vector<core_trajectory_msgs::TrajectoryXYZVYaw>
  trajectories){ std::vector< std::vector<double> > values(trajectories.size());
  ...
  dist*direction_vectors[m];//tf::Transform(q_curr)*(dist*direction); geometry_msgs::PoseStamped
  check_pose; check_pose.header = trajectory.header; check_pose.pose.position.x = point.x();
...
double DisparityMapRepresentation::distance_to_obstacle(geometry_msgs::msg::PoseStamped pose,
    ...
    double occupancy;
    if (!disp_graph->isStateValidDepth_pose(pose, 0.9, occupancy)) return 0.f;
...
pose.pose.position.z); point_target_frame = pose_to_target_frame_tf*point_pose_frame;
  ...
  for(int i = 0; i < image_pairs.size(); i++){
    ...
    //ROS_INFO_STREAM(point_image_frame.x() << " " << point_image_frame.y() << " " <<
point_image_frame.z() << " " << c << " " << r);
