...
Waypoint::Waypoint(double x, double y, double z, double yaw,
		   ...
		   double jx, double jy, double jz, double time){
  x_ = x;
  y_ = y;
  z_ = z;
  vx_ = vx;
...
tf2::Vector3 Waypoint::position() const {
  return tf2::Vector3(x_, y_, z_);
}

tf2::Vector3 Waypoint::velocity() const {
  return tf2::Vector3(vx_, vy_, vz_);
}

tf2::Vector3 Waypoint::acceleration() const {
  return tf2::Vector3(ax_, ay_, az_);
}

tf2::Vector3 Waypoint::jerk() const {
  return tf2::Vector3(jx_, jy_, jz_);
}

airstack_msgs::msg::Odometry Waypoint::odometry(rclcpp::Time stamp, std::string frame_id) const {
...
Waypoint Waypoint::interpolate(Waypoint wp, double t){
  ...
  double time_interp = time() + t*(wp.time() - time());

  Waypoint wp_interp(pos_interp.x(), pos_interp.y(), pos_interp.z(),
		     tf2::getYaw(q_interp),
		     vel_interp.x(), vel_interp.y(), vel_interp.z(),
		     accel_interp.x(), accel_interp.y(), accel_interp.z(),
		     jerk_interp.x(), jerk_interp.y(), jerk_interp.z(),
		     time_interp);
...
std::ostream& operator<<(std::ostream& os, const Waypoint& wp){
  return os << "[pos: " << wp.x() << ", " << wp.y() << ", " << wp.z()
	    << " vel: " << wp.vx() << ", " << wp.vy() << ", " << wp.vz()
	    << " acc: " << wp.ax() << ", " << wp.ay() << ", " << wp.az()
	    << " jerk: " << wp.jx() << ", " << wp.jy() << ", " << wp.jz()
...
Trajectory::Trajectory(rclcpp::Node* node, airstack_msgs::msg::TrajectoryXYZVYaw path){
  ...
  for(int i = 0; i < cleaned_path.waypoints.size(); i++){
	...
	wp2 = tf2::Vector3(cleaned_path.waypoints[i+1].position.x, cleaned_path.waypoints[i+1].position.y, cleaned_path.waypoints[i+1].position.z);
      else
	same = true;
    }
    else{ // otherwise use the previous waypoint
      wp1 = tf2::Vector3(cleaned_path.waypoints[i-1].position.x, cleaned_path.waypoints[i-1].position.y, cleaned_path.waypoints[i-1].position.z);
    }
    ...
    Waypoint waypoint(wp.position.x, wp.position.y, wp.position.z, wp.yaw, vel.x(), vel.y(), vel.z(),
		      wp.acceleration.x, wp.acceleration.y, wp.acceleration.z,
...
void Trajectory::generate_waypoint_times(){
  ...
  for(int i = 1; i < waypoints.size(); i++){
    ...
    //ROS_INFO_STREAM(i << " wp vel: " << curr_wp.velocity().x() << ", " << curr_wp.velocity().y() << ", " << curr_wp.velocity().z()
    //	    << " | " << prev_wp.velocity().x() << ", " << prev_wp.velocity().y() << ", " << prev_wp.velocity().z());
    ...
    double velocity = std::max(0.01, (curr_wp.velocity().length() + prev_wp.velocity().length())/2.);
...
bool Trajectory::get_closest_point(tf2::Vector3 point, Waypoint* closest, int* wp_index, double* path_distance){
...
bool Trajectory::merge(Trajectory traj, double min_time){
  ...
  std::cout << "COULDN'T MERGE BECAUSE OF min_time: " << closest_waypoint.time() << " " << min_time;
  return false;
}

bool Trajectory::get_closest_waypoint(tf2::Vector3 point, double initial_time, double end_time, Waypoint* waypoint){
  double closest_distance = std::numeric_limits<double>::max();
  bool found = false;
  

  for(int i = 1; i < waypoints.size(); i++){
    if(waypoints[i].time() < initial_time)
      continue;
    if(waypoints[i-1].time() > end_time)
      break;
    ...
    if(wp_start.time() < initial_time)
      wp_start = wp_start.interpolate(wp_end, (initial_time - wp_start.time())/(wp_end.time() - wp_start.time()));
    // handle the case that the end_time is between waypoint i-1 and waypoint i
    if(wp_end.time() > end_time)
      wp_end = wp_start.interpolate(wp_end, (end_time - wp_start.time())/(wp_end.time() - wp_start.time()));
...
bool Trajectory::get_waypoint_distance_ahead(double initial_time, double distance, Waypoint* waypoint){
  ...
  for(int i = 1; i < waypoints.size(); i++){
    ...
    if(current_distance + segment_distance >= distance){
      *waypoint = wp_start.interpolate(wp_end, (distance - current_distance)/segment_distance);
      return true;
    }
    else
      current_distance += segment_distance;

    if(i == waypoints.size()-1 && wp_end.time() >= initial_time){
      *waypoint = wp_end;
      return true;
    }
  }
...
bool Trajectory::get_waypoint_sphere_intersection(double initial_time, double ahead_distance, double time_end,
  ...
  for(int i = 1; i < waypoints.size(); i++){
    ...
    if(current_path_distance + segment_distance >= ahead_distance){
      should_break = true;
      wp_end = wp_start.interpolate(wp_end, (ahead_distance - current_path_distance)/segment_distance);
      if(end_waypoint != NULL)
	*end_waypoint = wp_end;
    }
    else if(wp_end.time()> time_end){
      should_break=true;
      wp_end = wp_start.interpolate(wp_end, (time_end - wp_start.time()) / (wp_end.time() - wp_start.time()) );
      if(end_waypoint!= NULL)
        *end_waypoint = wp_end;
    }
    else
    ...
    // sphere line intersection equations: http://www.ambrsoft.com/TrigoCalc/Sphere/SpherLineIntersection_.htm
    double x1 = wp_start.x();
    double y1 = wp_start.y();
    double z1 = wp_start.z();
    double x2 = wp_end.x();
    double y2 = wp_end.y();
    double z2 = wp_end.z();
    ...
    /*
    ROS_INFO_STREAM("xyz1: " << x1 << ", " << y1 << ", " << z1 << " xyz2: " << x2 << ", " << y2 << ", " << z2
		    << " c: " << xc << ", " << yc << ", " << zc << " abc: " << a << ", " << b << ", " << c
...
bool Trajectory::get_waypoint(double time, Waypoint* waypoint){
  ...
  for(int i = 1; i < waypoints.size(); i++){
    ...
    *waypoint = wp_start.interpolate(wp_end, (time - wp_start.time())/(wp_end.time() - wp_start.time()));
    return true;
  }
...
bool Trajectory::get_odom(double time, airstack_msgs::msg::Odometry* odom, rclcpp::Time stamp){
  ...
  if(waypoints.size() == 0){
    ...
    *odom = waypoints[0].odometry(stamp, frame_id);//rclcpp::Time::now(), frame_id);
    return true;
  }
  ...
  *odom = prev_wp.interpolate(curr_wp, t).odometry(stamp, frame_id);//rclcpp::Time::now(), frame_id);
  //ROS_INFO_STREAM("odom: " << odom->pose.position.x << ", " << odom->pose.position.y << ", " << odom->pose.position.z << " | "
...
Trajectory Trajectory::to_frame(std::string target_frame, rclcpp::Time time) {
  ...
  for(int i = 0; i < waypoints.size(); i++){
    ...
    tf2::Quaternion transformed_q = transform*wp.q();

    //ROS_INFO_STREAM("jerk: " << wp.jerk().x() << ", " << wp.jerk().y() << ", " << wp.jerk().z() << " | "
    //		    << transformed_jerk.x() << ", " << transformed_jerk.y() << ", " << transformed_jerk.z());
    
    Waypoint transformed_wp(transformed_position.x(), transformed_position.y(), transformed_position.z(),
			    tf2::getYaw(transformed_q),
			    transformed_velocity.x(), transformed_velocity.y(), transformed_velocity.z(),
			    transformed_acceleration.x(), transformed_acceleration.y(), transformed_acceleration.z(),
			    transformed_jerk.x(), transformed_jerk.y(), transformed_jerk.z());
...
Trajectory Trajectory::get_subtrajectory_distance(double start, double end){
  ...
  if(waypoints.size() > 0){
    ...
    if(start == 0. && waypoints.size() == 1)
      traj.waypoints.push_back(waypoints[0]);
...
float Trajectory::get_skip_ahead_time(float start_time, float max_velocity, float max_distance){
  ...
  for(int i = 1; i < waypoints.size(); i++){
    if(waypoints[i].time() < start_time)
      continue;
    
    Waypoint wp_start = waypoints[i-1];
    float wp_start_velocity = wp_start.velocity().length();
    if(wp_start_velocity > max_velocity)
      break;
    Waypoint wp_end = waypoints[i];
    float wp_end_velocity = wp_end.velocity().length();
    
    if(wp_start.time() < start_time)
      wp_start = wp_start.interpolate(wp_end, (start_time - wp_start.time())/(wp_end.time() - wp_start.time()));
    
    double segment_distance = wp_start.position().distance(wp_end.position());
    bool should_break = false;
    if(current_path_distance + segment_distance >= max_distance){
      should_break = true;
      wp_end = wp_start.interpolate(wp_end, (max_distance - current_path_distance)/segment_distance);
    }
    
    if(wp_end_velocity > max_velocity){
      should_break = true;
      wp_end = wp_start.interpolate(wp_end, std::min(1.f, std::max(0.f,
								   (max_velocity - wp_start_velocity)/
								   (wp_end_velocity - wp_start_velocity))));
    }

    skip_time = wp_end.time();
...
visualization_msgs::msg::MarkerArray Trajectory::get_markers(rclcpp::Time stamp, float r, float g, float b, float a, bool show_poses, bool show_velocity, float thickness){
...
airstack_msgs::msg::TrajectoryXYZVYaw TakeoffTrajectory::get_trajectory(airstack_msgs::msg::Odometry odom){
  ...
  tf2::Quaternion q(odom.pose.orientation.x,
  ...
  wp2.position.x = odom.pose.position.x + height*sin(path_pitch + (relative_to_orientation ? pitch : 0));
  wp2.position.y = odom.pose.position.y + height*sin(path_roll - (relative_to_orientation ? roll : 0));
...
AccelerationTrajectory::AccelerationTrajectory(tf2_ros::Buffer* buffer, std::string frame, double ax, double ay, double az, double dt, double ht, double max_velocity){
  //this->buffer = buffer;
...
airstack_msgs::msg::TrajectoryXYZVYaw AccelerationTrajectory::get_trajectory(airstack_msgs::msg::Odometry odom){
  airstack_msgs::msg::TrajectoryXYZVYaw traj;
  ...
  try{
    ...
    geometry_msgs::msg::TransformStamped t;
    t = buffer->lookupTransform(frame, odom.header.frame_id, rclcpp::Time(odom.header.stamp), rclcpp::Duration::from_seconds(0.1));
    tf2::fromMsg(t, pose_tf);
    //buffer->waitForTransform(frame, odom.header.frame_id, odom.header.stamp, rclcpp::Duration(0.1));
    //buffer->lookupTransform(frame, odom.header.frame_id, odom.header.stamp, pose_tf);
    tf2::Stamped<tf2::Transform> velocity_tf;
    t = buffer->lookupTransform(frame, odom.child_frame_id, rclcpp::Time(odom.header.stamp), rclcpp::Duration::from_seconds(0.1));
    tf2::fromMsg(t, velocity_tf);
    //buffer->waitForTransform(frame, odom.child_frame_id, odom.header.stamp, rclcpp::Duration(0.1));
    //buffer->lookupTransform(frame, odom.child_frame_id, odom.header.stamp, velocity_tf);
    velocity_tf.setOrigin(tf2::Vector3(0, 0, 0)); // only use rotation to transform the velocity
...
airstack_msgs::msg::TrajectoryXYZVYaw CurveTrajectory::get_trajectory(){
  return trajectory;
}
...
TrajectoryLibrary::TrajectoryLibrary(std::string config_filename, rclcpp::Node* node){//tf2_ros::Buffer* b){
  //if(buffer == NULL)
  //  buffer = b;
  //this->buffer = buffer;
  ...
  for(int i = 0; i < trajectories.size(); i++){
	...
	continue;
      ...
      AccelerationTrajectory* traj = new AccelerationTrajectory(buffer, frame, x, y, z, dt, ht, max_velocity);
...
std::vector<Trajectory> TrajectoryLibrary::get_dynamic_trajectories(airstack_msgs::msg::Odometry odom){
  std::vector<Trajectory> trajectories;
  ...
  for(int i = 0; i < dynamic_trajectories.size(); i++){
    ...
    if(path.waypoints.size() > 0)
      trajectories.push_back(Trajectory(node, path));
  }
